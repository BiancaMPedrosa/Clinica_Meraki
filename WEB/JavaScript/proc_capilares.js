document.addEventListener("DOMContentLoaded", () => {
  fetch("/WEB/JSON/conteudoPCAP.json")
    .then(res => res.json())
    .then(data => {
      preencherProcedimentos(data.procedimentosCapilares);
      preencherAntesDepois(data.antesDepois);
    })
    .catch(err => console.error("Erro ao carregar JSON:", err));
});

function preencherProcedimentos(lista) {
  const container = document.getElementById("procedimentosCapilares");
  container.innerHTML = "";
